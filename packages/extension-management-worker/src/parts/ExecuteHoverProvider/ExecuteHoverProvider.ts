...
const getMatchingExtensions = async (textDocument: TextDocument, assetDir: string, platform: number): Promise<readonly ExtensionManifest[]> => {
  ...
  return extensions.filter((extension): boolean => IsExtensionIsolated.isExtensionIsolated(extension) && contributesHoverProvider(extension, textDocument.languageId))
}
...
export const executeHoverProvider = async (textDocument: TextDocument, ...args: readonly unknown[]): Promise<unknown> => {
